...
Before start running this tool you need to prepare the input files:
...
This step receives the information of the input database and creates the working directory structure necessary for the following steps.

Note: you should indicate always the same output/working directory in all the steps.

Run ```bash create_project -h``` to see the usage of this command.
...
On this first search, only 1 random sequence per 10 in each orthogroups is used to get the associations, reducing the search space on the next step.
Here you also can define the threshold of identity percent you want to use. Default: 80.

Run ```bash relaxed_search -h``` to see the usage of this command.

## Restrictive Search

In this step, the tool performs a restrictive search only between the Orthogroups and groups of functions from the database that we find to be possibly related in the first step.

Run ```bash restrictive_search -h ``` to see the usage of this command.
...
Run ```bash annotation -h ``` to see the usage of this command.
...
Run ```bash create_db -h ``` to see the usage of this command.
